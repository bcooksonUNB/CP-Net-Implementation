...
unconditional_chi_square_value = 1.642374
conditional_chi_square_value = 1.642374
...
def conditional_chi_test(pat, pat2, total_counts, final_map):
    ...
    if chi_val >= conditional_chi_square_value: return True, chi_val
    return False, chi_val
...
def runMain(dir_name, CONDITIONAL_SUPPORT, remove_zeros=False, DEBUG=True, filePatternList=None, ratio=None):
    ...
    for pat in final_P_C_map:
        p_str = pat.replace(",",";") + "," + ("mal" if all_pattern_counts[pat][1]/all_pattern_counts[pat][0] >= ratio else "ben")
        w.write(p_str)
        for pat2 in final_P_C_map[pat]:
            c_str = "," + pat2.replace(",",";") + "," + final_category_map[pat][pat2]["PC"] + "," + final_category_map[pat][pat2]["PnC"] + "," + final_category_map[pat][pat2]["nPC"] + "," + final_category_map[pat][pat2]["nPnC"]
            w.write(c_str)
        w.write("\n")
