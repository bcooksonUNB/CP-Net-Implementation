...
def is_sequence_in_file(seq, file, support, win_size=None):
    if win_size == None:
        win_size = len(file)+1
    total_count = {x:0 for x in seq}
    pattern_count = 0
    current = 0
    win_count = 0
    f = 0
    last_index = -1
    while f < len(file):
        if current > 0: win_count += 1
        if file[f] == seq[current]: 
            current += 1
        if file[f] == seq[0]:
            last_index = f
            win_count = 0
...
def runMain(dir_name, MIN_SUPPORT, MAX_K, WINDOW_SIZE, DEBUG=True, pre_patterns=None, pre_filePatternList=None, pre_ratios=None):
    ...
    while True:
        ...
        for i in fileList:
            ...
            for c in cands:
                if is_sequence_in_file(c, fileList[i], win_size=WINDOW_SIZE):
