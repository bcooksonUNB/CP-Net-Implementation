...
def runMain(dir_name, CONDITIONAL_SUPPORT, remove_zeros=False, DEBUG=True, filePatternList=None, ratio=None):
    ...
    all_pattern_not_counts = {str(x):total_counts[str(x)] for x in total_not_counts}
        ...
        if str(pat) in all_pattern_counts:
            ...
            for name in file_name_list:
                ...
                if pat in file_pattern_list[name]:
                    ...
                    for pat2 in [x for x in all_pattern_counts.keys() if str(x) not in [str(y) for y in file_pattern_list[name]]]:
                        if str(pat2) in total_counts and str(pat2) != str(pat):
            ...
            for pat2 in final_map[str(pat)]:
                if float(final_map[str(pat)][str(pat2)][0])/float(all_pattern_counts[str(pat)][0]) < CONDITIONAL_SUPPORT:
            ...
            remove_list = []
            for pat2 in final_map[str(pat)]:
                if not conditional_chi_test(str(pat),str(pat2),all_pattern_counts,final_map)[0] or \
                    not conditional_chi_test(str(pat),str(pat2),all_pattern_not_counts,final_not_map)[0] or \
                    not unconditional_chi_test("pat", {"pat":final_map[str(pat)][str(pat2)]}, ratio)[0] or \
