...
def runMain(dir_name, CONDITIONAL_SUPPORT, remove_zeros=False, DEBUG=True, filePatternList=None, ratio=None):
    ...
    for pat in pattern_list:
        if str(pat) in all_pattern_counts:
            ...
            remove_list = []
            for pat2 in final_map[str(pat)]:
                if not unconditional_chi_test("pat", {"pat":final_map[str(pat)][str(pat2)]},ratio)[0] or \
                    not unconditional_chi_test("pat", {"pat":final_not_map[str(pat)][str(pat2)]},ratio)[0]:
                    remove_list.append(pat2)
            for p in remove_list:
                del final_map[str(pat)][str(p)]  
                del final_not_map[str(pat)][str(p)]      
    ...
    mal_map = {str(x):[{str(y):True if float(final_map[str(x)][str(y)][1])/final_map[str(x)][str(y)][0] >= ratio else False
                        for y in final_map[str(x)]},
                        {str(y):True if float(final_not_map[str(x)][str(y)][1])/final_not_map[str(x)][str(y)][0] >= ratio else False
                        for y in final_not_map[str(x)]}] for x in final_map}

    for pat in final_map:
        del_list = []
        for pat2 in final_map[pat]:
            if mal_map[str(pat)][0][str(pat2)] == mal_map[str(pat)][1][str(pat2)]:
    ...
    for pat in final_map:
        for pat2 in final_map:
            if pat2 in parents_list[pat] and pat in parents_list[pat2]:
                if unconditional_chi_test("pat", {"pat":final_map[str(pat)][str(pat2)]},ratio)[1] + unconditional_chi_test("pat", {"pat":final_not_map[str(pat)][str(pat2)]},ratio)[1] < \
                unconditional_chi_test("pat", {"pat":final_map[str(pat2)][str(pat)]},ratio)[1] + unconditional_chi_test("pat", {"pat":final_not_map[str(pat2)][str(pat)]},ratio)[1]:
                    del final_map[pat][pat2]
                    del final_not_map[pat][pat2]
                    parents_list[pat2].remove(pat)
                else:
                    del final_map[pat2][pat]
                    del final_not_map[pat2][pat]
            ...
            while not flag:
                ...
                if len(l) == 0:
                    ...
                    for node in range(len(l)-1):
                        chi = unconditional_chi_test("pat", {"pat":final_map[str(l[node+1])][str(l[node])]},ratio)[1] + unconditional_chi_test("pat", {"pat":final_not_map[str(l[node+1])][str(l[node])]},ratio)[1]
                        if chi < max:
                            max = chi
                            maxNode = node
                    del final_map[str(l[maxNode+1])][str(l[maxNode])]
                    del final_not_map[str(l[maxNode+1])][str(l[maxNode])]
    ...
    for pattern in final_map:
        ...
        for pat in final_map[str(pattern)]:
            ...
            write_string = "{0},{1},{2},{3},{4},{5},{6}\n".format(pat.replace(",",";"),final_map[str(pattern)][str(pat)][0],
                                                                ...
                                                                final_map[str(pattern)][str(pat)][2],final_not_map[str(pattern)][str(pat)][0],
                                                                final_not_map[str(pattern)][str(pat)][1],
                                                                final_not_map[str(pattern)][str(pat)][2])
            f.write(write_string)
        counter += 1
